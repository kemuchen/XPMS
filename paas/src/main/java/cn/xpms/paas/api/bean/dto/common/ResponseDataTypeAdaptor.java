 ...
 * @Version 1.0
 */
public class ResponseDataTypeAdaptor extends TypeAdapter<ApiResponseEntity> {

    public static final TypeAdapterFactory FACTORY = new TypeAdapterFactory() {
        @SuppressWarnings("unchecked")
        @Override
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
            if (type.getRawType() == ApiResponseEntity.class) {
    ...
    @Override
    public void write(JsonWriter out, ApiResponseEntity value) throws IOException {
        if (value == null) {
            out.nullValue();
            return;
        }
    ...
    @Override
    public ApiResponseEntity read(JsonReader in) throws IOException {
        ApiResponseEntity data = new ApiResponseEntity();
        Map<String, Object> dataMap = (Map<String, Object>) readInternal(in);
        data.setCode((String) dataMap.get("code"));
